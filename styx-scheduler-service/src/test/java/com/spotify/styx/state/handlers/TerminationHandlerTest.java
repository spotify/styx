...
public class TerminationHandlerTest {
  ...
  public void shouldCompleteOnZeroExitCode() throws Exception {
    ...
    var event = terminationHandler.transitionInto(zeroTerm).orElseThrow();
    assertThat(event, is(Event.success(WORKFLOW_INSTANCE)));
  }
  ...
  public void shouldScheduleRetryOnNonZero() throws Exception {
    ...
    var event = terminationHandler.transitionInto(nonZeroTerm).orElseThrow();
    assertThat(event, is(Event.retryAfter(WORKFLOW_INSTANCE, expectedDelay.toMillis())));
  }
  ...
  public void shouldScheduleRetryOnFail() throws Exception {
    ...
    Duration expectedDelay = Duration.ofMillis(4711);
    when(retryUtil.calculateDelay(anyInt())).thenReturn(expectedDelay);
    var event = terminationHandler.transitionInto(failed).orElseThrow();
    assertThat(event, is(Event.retryAfter(WORKFLOW_INSTANCE, expectedDelay.toMillis())));
  }
  ...
  public void shouldStopOnNonZeroMaxRetriesReached() throws Exception {
    ...
    var event = terminationHandler.transitionInto(maxedTerm).orElseThrow();
    assertThat(event, is(Event.stop(WORKFLOW_INSTANCE)));
  }
  ...
  public void shouldScheduleRetryOf10MinutesOnMissingDependencies() throws Exception {
    ...
    var event = terminationHandler.transitionInto(missingDeps).orElseThrow();
    assertThat(event, is(Event.retryAfter(WORKFLOW_INSTANCE, Duration.ofMinutes(10).toMillis())));
  }
